...
- [ ] 模型训练及仿真整体实现
      ...
      - [ ] 修改模型结构，加载一个模型得到加速度，并以此加速度更新所有的流体粒子
           ...
           1. - [x] 使用python训练网络模型，存入checkpoint文件，之后使用freeze_graph.p将模型转存入ptotobuf文件
            2. - [x] c++加载图及参数至session中，根据输入直接输出结果（但是目前对张量数据的加载仍未理解透彻，本来图片加载是一个很好的例子，目前仍需要继承三种方式，从array中直接加载，从csv中读取，从图片中提取）
               - [ ] 使用Armadillo，[**submatrix views**](http://arma.sourceforge.net/docs.html#submat)，矩阵操作和python几乎能够无缝对接，但是有一点比较恼人，armadillo似乎是列存储的
            3. - [ ] 结合仿真程序加载模型并反馈网络的输出结果
               - [ ] 后续仍需要调整数据集，进行交叉验证，降低数据的过拟合，这样就可以设定场景即时开始仿真了
...
损失函数
$$
L = α \cfrac{1}{N_{pos}}􏰇 \sum_i L_{cls}(p^{pos}_i, 1) + β\cfrac1{N_{neg}}\sum_j 􏰇 L_{cls}(p_j^{neg}, 0)+ \cfrac1{N_{pos}} 􏰇\sum_iL_{reg}(u_i,u^∗_i)
$$
...
- [ ] 将分组的代码以c++形式实现
      - [x] c++矩阵运算
      - [x] 将矩阵转换为tensorflow所需要的tensor形式，可以直接复制
      - [ ] 目前最耗时的部分在于未每一个粒子以其中心坐标生成特征向量
            - [x] 已经将每个粒子的坐标处理转化到在网络中进行前向处理；
            - [x] 那么在c++中，就需要将feed_dict里所需要的变量都转化为tensor，首先需要找到它们在graph中的node name
            - [ ] 一事不明，在forward的过程中，受batch_size的影响吗，batch_size的值是受训练时的参数固定的吗
...
- [ ] 驱动摘要和综述
      ...
      - [ ] 摘要
      - [ ] 综述
            - [ ] 重点查阅一下最新是否还有流体相关的驱动论文，无论是粒子法还是网格法，都可以进行论述
            - [ ] 3-D数据的检测相关论文也可以写入综述中，嗯，以及在目标检测中使用的中间特征提取的方式，交换不变性质的表现等等
...
虽然有很多功能或者你面临的问题，已经有人解决了，但是在此刻的你是不知道的，你不知道平行的那一边已经存在的方案，因为不同的场景下，同一个问题也会有变数，还是要去探索，因而还是要去实践，经世致用。